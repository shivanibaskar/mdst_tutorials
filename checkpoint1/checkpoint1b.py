...
def remove_percents(df, col):
    df[col]=df[col].str.extract(pat = '([0-9]+)')
    return df
...
def standardize_names(df):
    ...
    for column_name in df.columns:
        column_name_modified=column_name.lower().lstrip().rstrip()
        column_name_modified=re.sub(r"\([^()]*\)", "",column_name_modified)
        standardised_column_names.append(column_name_modified)
    df.columns =standardised_column_names
    return df

def fix_strings(df, col):
    df[col]=df[col].str.extract(pat = '([a-zA-Z]+)')
